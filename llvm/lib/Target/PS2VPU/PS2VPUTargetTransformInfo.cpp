//===-- PS2VPUTargetTransformInfo.cpp - PS2VPU specific TTI -------------===//
//
...
//#include "PS2VPUExpandImm.h"
//#include "PS2VPUPerfectShuffle.h"
#include "PS2VPUTargetTransformInfo.h"
//#include "MCTargetDesc/PS2VPUAddressingModes.h"
...
//#include "llvm/IR/IntrinsicsPS2VPU.h"
...
static bool hasPossibleIncompatibleOps(const Function *F) {
  for (const BasicBlock &BB : *F) {
    for (const Instruction &I : BB) {
      ...
      // @llvm.PS2VPU.* or @llvm.gather/scatter intrinsics). We can assume that
...
bool PS2VPUTTIImpl::areInlineCompatible(const Function *Caller,
...
bool PS2VPUTTIImpl::areTypesABICompatible(
...
unsigned
PS2VPUTTIImpl::getInlineCallPenalty(const Function *F, const CallBase &Call,
...
bool PS2VPUTTIImpl::shouldMaximizeVectorBandwidth(
...
InstructionCost PS2VPUTTIImpl::getIntImmCost(int64_t Val) {
  // Check if the immediate can be encoded within an instruction.
  //if (Val == 0 || PS2VPU_AM::isLogicalImmediate(Val, 64))
  //  return 0;
  ...
  //SmallVector<PS2VPU_IMM::ImmInsnModel, 4> Insn;
  //PS2VPU_IMM::expandMOVImm(Val, 64, Insn);
  //return Insn.size();
  return 1;
}

/// Calculate the cost of materializing the given constant.
InstructionCost PS2VPUTTIImpl::getIntImmCost(const APInt &Imm, Type *Ty,
...
InstructionCost PS2VPUTTIImpl::getIntImmCostInst(unsigned Opcode, unsigned Idx,
  ...
  //  InstructionCost Cost = PS2VPUTTIImpl::getIntImmCost(Imm, Ty, CostKind);
  //  return (Cost <= NumConstants * TTI::TCC_Basic)
  //             ? static_cast<int>(TTI::TCC_Free)
  //             : Cost;
  //}
  //return PS2VPUTTIImpl::getIntImmCost(Imm, Ty, CostKind);
  return 1;
}

InstructionCost
PS2VPUTTIImpl::getIntImmCostIntrin(Intrinsic::ID IID, unsigned Idx,
                                    const APInt &Imm, Type *Ty,
                                    TTI::TargetCostKind CostKind) {
  assert(Ty->isIntegerTy());
  ...
  //// Most (all?) PS2VPU intrinsics do not support folding immediates into the
  //// selected instruction, so we compute the materialization cost for the
  //// immediate directly.
  //if (IID >= Intrinsic::PS2VPU_addg && IID <= Intrinsic::PS2VPU_udiv)
  //  return PS2VPUTTIImpl::getIntImmCost(Imm, Ty, CostKind);
...
PS2VPUTTIImpl::getPopcntSupport(unsigned TyWidth) {
  assert(isPowerOf2_32(TyWidth) && "Ty width must be power of 2");
  if (TyWidth == 32 || TyWidth == 64)
    return TTI::PSK_FastHardware;
  // TODO: PS2VPUTargetLowering::LowerCTPOP() supports 128bit popcount.
  return TTI::PSK_Software;
}

InstructionCost
PS2VPUTTIImpl::getIntrinsicInstrCost(const IntrinsicCostAttributes &ICA,
                                      TTI::TargetCostKind CostKind) {
  ...
  for (Value *IncValPhi : PN->incoming_values()) {
    ...
    /*if (!Reinterpret ||
        Reinterpret->getIntrinsicID() !=
            Intrinsic::PS2VPU_sve_convert_to_svbool ||
...
tryCombineFromSVBoolBinOp(InstCombiner &IC, IntrinsicInst &II) {
  ...
  switch (IntrinsicID) {
  /*case Intrinsic::PS2VPU_sve_and_z:
  case Intrinsic::PS2VPU_sve_bic_z:
  case Intrinsic::PS2VPU_sve_eor_z:
  case Intrinsic::PS2VPU_sve_nand_z:
  case Intrinsic::PS2VPU_sve_nor_z:
  case Intrinsic::PS2VPU_sve_orn_z:
  case Intrinsic::PS2VPU_sve_orr_z:
    break;*/
  default:
    return std::nullopt;
  }
  ...
  /*if (!PredIntr ||
      PredIntr->getIntrinsicID() != Intrinsic::PS2VPU_sve_convert_to_svbool)*/
    return std::nullopt;
  ...
  auto NarrowBinOpOp1 = IC.Builder.CreateIntrinsic(
      Intrinsic::PS2VPU_sve_convert_from_svbool, {PredOpTy}, {BinOpOp1});
  NarrowedBinOpArgs.push_back(NarrowBinOpOp1);
  if (BinOpOp1 == BinOpOp2)
    NarrowedBinOpArgs.push_back(NarrowBinOpOp1);
  else
    NarrowedBinOpArgs.push_back(IC.Builder.CreateIntrinsic(
        Intrinsic::PS2VPU_sve_convert_from_svbool, {PredOpTy}, {BinOpOp2}));
  ...
  while (Cursor) {
    ...
    /*if (!IntrinsicCursor || !(IntrinsicCursor->getIntrinsicID() ==
                              ...
                              IntrinsicCursor->getIntrinsicID() ==
                                  Intrinsic::PS2VPU_sve_convert_from_svbool))*/
      break;
...
static bool isAllActivePredicate(Value *Pred) {
  ...
  Value *UncastedPred;
  //if (match(Pred, m_Intrinsic<Intrinsic::PS2VPU_sve_convert_from_svbool>(
  //                    m_Intrinsic<Intrinsic::PS2VPU_sve_convert_to_svbool>(
  ...
  //return match(Pred, m_Intrinsic<Intrinsic::PS2VPU_sve_ptrue>(
  //                       m_ConstantInt<PS2VPUSVEPredPattern::all>()));
  return false;
}
...
static std::optional<Instruction *> instCombineSVEDup(InstCombiner &IC,
  ...
  //if (Pg->getIntrinsicID() != Intrinsic::PS2VPU_sve_ptrue)
  //  return std::nullopt;

  //const auto PTruePattern =
  //    cast<ConstantInt>(Pg->getOperand(0))->getZExtValue();
  //if (PTruePattern != PS2VPUSVEPredPattern::vl1)
  //  return std::nullopt;
...
//  if (!Pg || Pg->getIntrinsicID() != Intrinsic::PS2VPU_sve_ptrue)
//    return std::nullopt;
//
//  const auto PTruePattern =
//      cast<ConstantInt>(Pg->getOperand(0))->getZExtValue();
//  if (PTruePattern != PS2VPUSVEPredPattern::all)
//    return std::nullopt;
//
...
//  if (!DupQLane ||
//      DupQLane->getIntrinsicID() != Intrinsic::PS2VPU_sve_dupq_lane)
//    return std::nullopt;
//
...
//      Type::getInt1Ty(Ctx), PS2VPU::SVEBitsPerBlock / (PredSize * 8));
//
...
//
//  auto *PTruePat =
//      ConstantInt::get(Type::getInt32Ty(Ctx), PS2VPUSVEPredPattern::all);
//  auto *PTrue = IC.Builder.CreateIntrinsic(Intrinsic::PS2VPU_sve_ptrue,
//                                           {PredType}, {PTruePat});
//  auto *ConvertToSVBool = IC.Builder.CreateIntrinsic(
//      Intrinsic::PS2VPU_sve_convert_to_svbool, {PredType}, {PTrue});
//  auto *ConvertFromSVBool =
//      IC.Builder.CreateIntrinsic(Intrinsic::PS2VPU_sve_convert_from_svbool,
...
//  bool IsAfter = IntrinsicID == Intrinsic::PS2VPU_sve_lasta;
//
//  // lastX(splat(X)) --> X
...
//    return std::nullopt;
//
//  if (IntrPG->getIntrinsicID() != Intrinsic::PS2VPU_sve_ptrue)
//    return std::nullopt;
//
//  const auto PTruePattern =
...
//                                           {II.getType()}, {AllPat});
//  auto *RDFFR =
//      IC.Builder.CreateIntrinsic(Intrinsic::PS2VPU_sve_rdffr_z, {}, {PTrue});
//  RDFFR->takeName(&II);
...
//
//  if (Pattern == PS2VPUSVEPredPattern::all) {
...
//  if (PgVal == OpVal &&
//      (II.getIntrinsicID() == Intrinsic::PS2VPU_sve_ptest_first ||
//       II.getIntrinsicID() == Intrinsic::PS2VPU_sve_ptest_last)) {
//    Value *Ops[] = {PgVal, OpVal};
//    Type *Tys[] = {PgVal->getType()};
//
//    auto *PTest =
//        IC.Builder.CreateIntrinsic(Intrinsic::PS2VPU_sve_ptest_any, Tys, Ops);
//    PTest->takeName(&II);
//
...
//
//  if (Pg->getIntrinsicID() == Intrinsic::PS2VPU_sve_convert_to_svbool &&
//      OpIID == Intrinsic::PS2VPU_sve_convert_to_svbool &&
...
//  if ((Pg == Op) && (II.getIntrinsicID() == Intrinsic::PS2VPU_sve_ptest_any) &&
//      ((OpIID == Intrinsic::PS2VPU_sve_brka_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_brkb_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_brkpa_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_brkpb_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_rdffr_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_and_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_bic_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_eor_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_nand_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_nor_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_orn_z) ||
//       (OpIID == Intrinsic::PS2VPU_sve_orr_z))) {
...
//  switch (Intrinsic) {
//  case Intrinsic::PS2VPU_sve_fmul_u:
//    return Instruction::BinaryOps::FMul;
//  case Intrinsic::PS2VPU_sve_fadd_u:
//    return Instruction::BinaryOps::FAdd;
//  case Intrinsic::PS2VPU_sve_fsub_u:
...
//      !match(OpPredicate, m_Intrinsic<Intrinsic::PS2VPU_sve_ptrue>(
//                              m_ConstantInt<PS2VPUSVEPredPattern::all>())))
//    return std::nullopt;
...
//                                                            Intrinsic::ID IID) {
//  auto *OpPredicate = II.getOperand(0);
//  if (!match(OpPredicate, m_Intrinsic<Intrinsic::PS2VPU_sve_ptrue>(
//                              m_ConstantInt<PS2VPUSVEPredPattern::all>())))
//    return std::nullopt;
//
...
//                                                            IntrinsicInst &II) {
//  if (auto II_U = instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_add_u))
//    return II_U;
//  if (auto MLA = instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_mul,
//                                                   Intrinsic::PS2VPU_sve_mla>(
//          IC, II, true))
//    return MLA;
//  if (auto MAD = instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_mul,
//                                                   Intrinsic::PS2VPU_sve_mad>(
//          IC, II, false))
//    return MAD;
//  return std::nullopt;
//}
...
//  if (auto II_U = instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fadd_u))
//    return II_U;
//  if (auto FMLA =
//          instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_fmul,
//                                            Intrinsic::PS2VPU_sve_fmla>(IC, II,
//                                                                         true))
//    return FMLA;
//  if (auto FMAD =
//          instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_fmul,
//                                            Intrinsic::PS2VPU_sve_fmad>(IC, II,
//                                                                         false))
//    return FMAD;
//  if (auto FMLA =
//          instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_fmul_u,
//                                            Intrinsic::PS2VPU_sve_fmla>(IC, II,
//                                                                         true))
//    return FMLA;
//  return std::nullopt;
...
//  if (auto II_U = instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fsub_u))
//    return II_U;
//  if (auto FMLS =
//          instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_fmul,
//                                            Intrinsic::PS2VPU_sve_fmls>(IC, II,
//                                                                         true))
//    return FMLS;
//  if (auto FMSB =
//          instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_fmul,
//                                            Intrinsic::PS2VPU_sve_fnmsb>(
//              IC, II, false))
//    return FMSB;
//  if (auto FMLS =
//          instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_fmul_u,
...
//                                                            IntrinsicInst &II) {
//  if (auto II_U = instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_sub_u))
//    return II_U;
//  if (auto MLS = instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_mul,
//                                                   Intrinsic::PS2VPU_sve_mls>(
//          IC, II, true))
//    return MLS;
//  return std::nullopt;
//}
...
//  };
//
//  // Return true if a given instruction is an PS2VPU_sve_dup intrinsic call
//  // with a unit splat value, false otherwise.
//  auto IsUnitDup = [](auto *I) {
//    auto *IntrI = dyn_cast<IntrinsicInst>(I);
//    if (!IntrI || IntrI->getIntrinsicID() != Intrinsic::PS2VPU_sve_dup)
//      return false;
//
...
//  auto *RetTy = cast<ScalableVectorType>(II.getType());
//  bool IsSigned = II.getIntrinsicID() == Intrinsic::PS2VPU_sve_sunpkhi ||
//                  II.getIntrinsicID() == Intrinsic::PS2VPU_sve_sunpklo;
//
...
static std::optional<Instruction *> instCombineSVEZip(InstCombiner &IC,
  ...
  /*if (match(II.getArgOperand(0),
            m_Intrinsic<Intrinsic::PS2VPU_sve_uzp1>(m_Value(A), m_Value(B))) &&
      match(II.getArgOperand(1), m_Intrinsic<Intrinsic::PS2VPU_sve_uzp2>(
                                     m_Specific(A), m_Specific(B))))
    return IC.replaceInstUsesWith(
        II, (II.getIntrinsicID() == Intrinsic::PS2VPU_sve_zip1 ? A : B));*/
...
instCombineLD1GatherIndex(InstCombiner &IC, IntrinsicInst &II) {
  ...
  Value *IndexBase;
  /*if (match(Index, m_Intrinsic<Intrinsic::PS2VPU_sve_index>(
...
//        Intrinsic::PS2VPU_sve_asrd, {II.getType()}, {Pred, Vec, DivisorLog2});
...
//        Intrinsic::PS2VPU_sve_neg, {ASRD->getType()}, {ASRD, Pred, ASRD});
...
//  if (!match(Vec, m_Intrinsic<Intrinsic::PS2VPU_sve_sqabs>(
//                      m_Value(MergedValue), m_Value(AbsPred), m_Value())) &&
//      !match(Vec, m_Intrinsic<Intrinsic::PS2VPU_sve_abs>(
//                      m_Value(MergedValue), m_Value(AbsPred), m_Value())))
//
//    return std::nullopt;
//
...
//    return std::nullopt;
//
//  auto LSL = IC.Builder.CreateIntrinsic(Intrinsic::PS2VPU_sve_lsl,
...
PS2VPUTTIImpl::instCombineIntrinsic(InstCombiner &IC,
                                     IntrinsicInst &II) const {
  Intrinsic::ID IID = II.getIntrinsicID();
  switch (IID) {
  default:
    break;
  /*case Intrinsic::PS2VPU_neon_fmaxnm:
  case Intrinsic::PS2VPU_neon_fminnm:
    return instCombineMaxMinNM(IC, II);
  case Intrinsic::PS2VPU_sve_convert_from_svbool:
    return instCombineConvertFromSVBool(IC, II);
  case Intrinsic::PS2VPU_sve_dup:
    return instCombineSVEDup(IC, II);
  case Intrinsic::PS2VPU_sve_dup_x:
    return instCombineSVEDupX(IC, II);
  case Intrinsic::PS2VPU_sve_cmpne:
  case Intrinsic::PS2VPU_sve_cmpne_wide:
    return instCombineSVECmpNE(IC, II);
  case Intrinsic::PS2VPU_sve_rdffr:
    return instCombineRDFFR(IC, II);
  case Intrinsic::PS2VPU_sve_lasta:
  case Intrinsic::PS2VPU_sve_lastb:
    return instCombineSVELast(IC, II);
  case Intrinsic::PS2VPU_sve_clasta_n:
  case Intrinsic::PS2VPU_sve_clastb_n:
    return instCombineSVECondLast(IC, II);
  case Intrinsic::PS2VPU_sve_cntd:
    return instCombineSVECntElts(IC, II, 2);
  case Intrinsic::PS2VPU_sve_cntw:
    return instCombineSVECntElts(IC, II, 4);
  case Intrinsic::PS2VPU_sve_cnth:
    return instCombineSVECntElts(IC, II, 8);
  case Intrinsic::PS2VPU_sve_cntb:
    return instCombineSVECntElts(IC, II, 16);
  case Intrinsic::PS2VPU_sve_ptest_any:
  case Intrinsic::PS2VPU_sve_ptest_first:
  case Intrinsic::PS2VPU_sve_ptest_last:
    return instCombineSVEPTest(IC, II);
  case Intrinsic::PS2VPU_sve_fabd:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fabd_u);
  case Intrinsic::PS2VPU_sve_fadd:
    return instCombineSVEVectorFAdd(IC, II);
  case Intrinsic::PS2VPU_sve_fadd_u:
    return instCombineSVEVectorFAddU(IC, II);
  case Intrinsic::PS2VPU_sve_fdiv:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fdiv_u);
  case Intrinsic::PS2VPU_sve_fmax:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fmax_u);
  case Intrinsic::PS2VPU_sve_fmaxnm:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fmaxnm_u);
  case Intrinsic::PS2VPU_sve_fmin:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fmin_u);
  case Intrinsic::PS2VPU_sve_fminnm:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fminnm_u);
  case Intrinsic::PS2VPU_sve_fmla:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fmla_u);
  case Intrinsic::PS2VPU_sve_fmls:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fmls_u);
  case Intrinsic::PS2VPU_sve_fmul:
  case Intrinsic::PS2VPU_sve_fmul_u:
    return instCombineSVEVectorMul(IC, II, Intrinsic::PS2VPU_sve_fmul_u);
  case Intrinsic::PS2VPU_sve_fmulx:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fmulx_u);
  case Intrinsic::PS2VPU_sve_fnmla:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fnmla_u);
  case Intrinsic::PS2VPU_sve_fnmls:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_fnmls_u);
  case Intrinsic::PS2VPU_sve_fsub:
    return instCombineSVEVectorFSub(IC, II);
  case Intrinsic::PS2VPU_sve_fsub_u:
    return instCombineSVEVectorFSubU(IC, II);
  case Intrinsic::PS2VPU_sve_add:
    return instCombineSVEVectorAdd(IC, II);
  case Intrinsic::PS2VPU_sve_add_u:
    return instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_mul_u,
                                             Intrinsic::PS2VPU_sve_mla_u>(
        IC, II, true);
  case Intrinsic::PS2VPU_sve_mla:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_mla_u);
  case Intrinsic::PS2VPU_sve_mls:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_mls_u);
  case Intrinsic::PS2VPU_sve_mul:
  case Intrinsic::PS2VPU_sve_mul_u:
    return instCombineSVEVectorMul(IC, II, Intrinsic::PS2VPU_sve_mul_u);
  case Intrinsic::PS2VPU_sve_sabd:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_sabd_u);
  case Intrinsic::PS2VPU_sve_smax:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_smax_u);
  case Intrinsic::PS2VPU_sve_smin:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_smin_u);
  case Intrinsic::PS2VPU_sve_smulh:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_smulh_u);
  case Intrinsic::PS2VPU_sve_sub:
    return instCombineSVEVectorSub(IC, II);
  case Intrinsic::PS2VPU_sve_sub_u:
    return instCombineSVEVectorFuseMulAddSub<Intrinsic::PS2VPU_sve_mul_u,
                                             Intrinsic::PS2VPU_sve_mls_u>(
        IC, II, true);
  case Intrinsic::PS2VPU_sve_uabd:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_uabd_u);
  case Intrinsic::PS2VPU_sve_umax:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_umax_u);
  case Intrinsic::PS2VPU_sve_umin:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_umin_u);
  case Intrinsic::PS2VPU_sve_umulh:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_umulh_u);
  case Intrinsic::PS2VPU_sve_asr:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_asr_u);
  case Intrinsic::PS2VPU_sve_lsl:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_lsl_u);
  case Intrinsic::PS2VPU_sve_lsr:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_lsr_u);
  case Intrinsic::PS2VPU_sve_and:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_and_u);
  case Intrinsic::PS2VPU_sve_bic:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_bic_u);
  case Intrinsic::PS2VPU_sve_eor:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_eor_u);
  case Intrinsic::PS2VPU_sve_orr:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_orr_u);
  case Intrinsic::PS2VPU_sve_sqsub:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_sqsub_u);
  case Intrinsic::PS2VPU_sve_uqsub:
    return instCombineSVEAllActive(II, Intrinsic::PS2VPU_sve_uqsub_u);
  case Intrinsic::PS2VPU_sve_tbl:
    return instCombineSVETBL(IC, II);
  case Intrinsic::PS2VPU_sve_uunpkhi:
  case Intrinsic::PS2VPU_sve_uunpklo:
  case Intrinsic::PS2VPU_sve_sunpkhi:
  case Intrinsic::PS2VPU_sve_sunpklo:
    return instCombineSVEUnpack(IC, II);
  case Intrinsic::PS2VPU_sve_zip1:
  case Intrinsic::PS2VPU_sve_zip2:
    return instCombineSVEZip(IC, II);
  case Intrinsic::PS2VPU_sve_ld1_gather_index:
    return instCombineLD1GatherIndex(IC, II);
  case Intrinsic::PS2VPU_sve_st1_scatter_index:
    return instCombineST1ScatterIndex(IC, II);
  case Intrinsic::PS2VPU_sve_ld1:
    return instCombineSVELD1(IC, II, DL);
  case Intrinsic::PS2VPU_sve_st1:
    return instCombineSVEST1(IC, II, DL);
  case Intrinsic::PS2VPU_sve_sdiv:
    return instCombineSVESDIV(IC, II);
  case Intrinsic::PS2VPU_sve_sel:
    return instCombineSVESel(IC, II);
  case Intrinsic::PS2VPU_sve_srshl:
    return instCombineSVESrshl(IC, II);
  case Intrinsic::PS2VPU_sve_dupq_lane:
...
std::optional<Value *> PS2VPUTTIImpl::simplifyDemandedVectorEltsIntrinsic(
  ...
  default:
    break;
  /*case Intrinsic::PS2VPU_neon_fcvtxn:
  case Intrinsic::PS2VPU_neon_rshrn:
  case Intrinsic::PS2VPU_neon_sqrshrn:
  case Intrinsic::PS2VPU_neon_sqrshrun:
  case Intrinsic::PS2VPU_neon_sqshrn:
  case Intrinsic::PS2VPU_neon_sqshrun:
  case Intrinsic::PS2VPU_neon_sqxtn:
  case Intrinsic::PS2VPU_neon_sqxtun:
  case Intrinsic::PS2VPU_neon_uqrshrn:
  case Intrinsic::PS2VPU_neon_uqshrn:
  case Intrinsic::PS2VPU_neon_uqxtn:
...
TypeSize
PS2VPUTTIImpl::getRegisterBitWidth(TargetTransformInfo::RegisterKind K) const {
  switch (K) {
...
bool PS2VPUTTIImpl::isWideningInstruction(Type *DstTy, unsigned Opcode,
...
//
bool PS2VPUTTIImpl::isExtPartOfAvgExpr(const Instruction *ExtUser, Type *Dst,
                                        Type *Src) {
...
InstructionCost PS2VPUTTIImpl::getCastInstrCost(unsigned Opcode, Type *Dst,
                                                 Type *Src,
  ...
  /*if (SrcTy.isFixedLengthVector() && DstTy.isFixedLengthVector() &&
    ...
    unsigned NumElements = PS2VPU::SVEBitsPerBlock /
  ...
  if ((ISD == ISD::ZERO_EXTEND || ISD == ISD::SIGN_EXTEND) &&
      CCH == TTI::CastContextHint::Masked && false/*ST->hasSVEorSME()*/ &&
    ...
    InstructionCost Part1 = PS2VPUTTIImpl::getCastInstrCost(
        Opcode, LegalTy, Src, CCH, CostKind, I);
    InstructionCost Part2 = PS2VPUTTIImpl::getCastInstrCost(
...
InstructionCost PS2VPUTTIImpl::getExtractWithExtendCost(unsigned Opcode,
                                                         Type *Dst,
                                                         VectorType *VecTy,
                                                         unsigned Index) {
...
InstructionCost PS2VPUTTIImpl::getCFInstrCost(unsigned Opcode,
                                               TTI::TargetCostKind CostKind,
                                               const Instruction *I) {
  if (CostKind != TTI::TCK_RecipThroughput)
...
InstructionCost PS2VPUTTIImpl::getVectorInstrCostHelper(const Instruction *I,
                                                         Type *Val,
                                                         unsigned Index,
                                                         bool HasRealUse) {
  ...
  if (Index != -1U) {
    ...
    if (I && dyn_cast<LoadInst>(I->getOperand(1)))
      return 2/*ST->getVectorInsertExtractBaseCost()*/ + 1;
  ...
  return 2/*ST->getVectorInsertExtractBaseCost()*/;
}

InstructionCost PS2VPUTTIImpl::getVectorInstrCost(unsigned Opcode, Type *Val,
                                                   TTI::TargetCostKind CostKind,
...
InstructionCost PS2VPUTTIImpl::getVectorInstrCost(const Instruction &I,
                                                   Type *Val,
                                                   TTI::TargetCostKind CostKind,
                                                   unsigned Index) {
...
InstructionCost PS2VPUTTIImpl::getScalarizationOverhead(
  ...
  //       2/*ST->getVectorInsertExtractBaseCost()*/;
  
    // not sure about that, there are no scalar float registers, so what does
  // insert/extract means?
  // ([x, u, u, u], [y, u, u, u]) => [x, y, u, u] maybe? cost is 1 rotate
  // what about insert for parameters, they are already vectors so cost is 0
  return 0;
}

InstructionCost PS2VPUTTIImpl::getArithmeticInstrCost(
  ...
  case ISD::SDIV:
    if (Op2Info.isConstant() && Op2Info.isUniform() && Op2Info.isPowerOf2()) {
      // On PS2VPU, scalar signed division by constants power-of-two are
  ...
  case ISD::UDIV: {
    ...
    if (Ty->isVectorTy()) {
      ...
      //} else 
      {
        ...
        // On PS2VPU, without SVE, vector divisions are expanded
...
InstructionCost PS2VPUTTIImpl::getAddressComputationCost(Type *Ty,
                                                          ScalarEvolution *SE,
                                                          const SCEV *Ptr) {
  ...
  unsigned NumVectorInstToHideOverhead = 10;//NeonNonConstStrideOverhead;
...
InstructionCost PS2VPUTTIImpl::getCmpSelInstrCost(unsigned Opcode, Type *ValTy,
                                                   Type *CondTy,
...
PS2VPUTTIImpl::TTI::MemCmpExpansionOptions
PS2VPUTTIImpl::enableMemCmpExpansion(bool OptSize, bool IsZeroCmp) const {
  ...
  //// TODO: Though vector loads usually perform well on PS2VPU, in some targets
...
bool PS2VPUTTIImpl::prefersVectorizedAddressing() const {
  return false/*ST->hasSVE()*/;
}

InstructionCost
PS2VPUTTIImpl::getMaskedMemoryOpCost(unsigned Opcode, Type *Src,
...
static unsigned getSVEGatherScatterOverhead(unsigned Opcode) {
  return 1/*Opcode == Instruction::Load ? SVEGatherOverhead : SVEScatterOverhead*/;
}

InstructionCost PS2VPUTTIImpl::getGatherScatterOpCost(
...
bool PS2VPUTTIImpl::useNeonVector(const Type *Ty) const {
  return isa<FixedVectorType>(Ty)/* && !ST->useSVEForFixedLengthVectors()*/;
}

InstructionCost PS2VPUTTIImpl::getMemoryOpCost(unsigned Opcode, Type *Ty,
...
InstructionCost PS2VPUTTIImpl::getInterleavedMemoryOpCost(
...
InstructionCost
PS2VPUTTIImpl::getCostOfKeepingLiveOverCall(ArrayRef<Type *> Tys) {
  InstructionCost Cost = 0;
  TTI::TargetCostKind CostKind = TTI::TCK_RecipThroughput;
  for (auto *I : Tys) {
    if (!I->isVectorTy())
...
unsigned PS2VPUTTIImpl::getMaxInterleaveFactor(ElementCount VF) {
  return 2;
  //ST->getMaxInterleaveFactor();
}
...
void PS2VPUTTIImpl::getUnrollingPreferences(Loop *L, ScalarEvolution &SE,
  ...
  /*if (ST->getProcFamily() == PS2VPUSubtarget::Falkor &&
  ...
  // If mcpu is omitted, getProcFamily() returns PS2VPUSubtarget::Others, so by
  // checking for that case, we can ensure that the default behaviour is
  // unchanged
  /*if (ST->getProcFamily() != PS2VPUSubtarget::Others &&
...
void PS2VPUTTIImpl::getPeelingPreferences(Loop *L, ScalarEvolution &SE,
                                           TTI::PeelingPreferences &PP) {
  BaseT::getPeelingPreferences(L, SE, PP);
}

Value *PS2VPUTTIImpl::getOrCreateResultFromMemIntrinsic(IntrinsicInst *Inst,
                                                         Type *ExpectedType) {
  switch (Inst->getIntrinsicID()) {
  default:
    return nullptr;
  //case Intrinsic::PS2VPU_neon_st2:
  //case Intrinsic::PS2VPU_neon_st3:
  //case Intrinsic::PS2VPU_neon_st4: {
  ...
  //  return Res;
  //}
  /*case Intrinsic::PS2VPU_neon_ld2:
  case Intrinsic::PS2VPU_neon_ld3:
  case Intrinsic::PS2VPU_neon_ld4:
...
bool PS2VPUTTIImpl::getTgtMemIntrinsic(IntrinsicInst *Inst,
...
bool PS2VPUTTIImpl::shouldConsiderAddressTypePromotion(
...
bool PS2VPUTTIImpl::isLegalToVectorizeReduction(
...
InstructionCost
PS2VPUTTIImpl::getMinMaxReductionCost(Intrinsic::ID IID, VectorType *Ty,
                                       FastMathFlags FMF,
                                       TTI::TargetCostKind CostKind) {
  std::pair<InstructionCost, MVT> LT = getTypeLegalizationCost(Ty);
...
InstructionCost PS2VPUTTIImpl::getArithmeticReductionCostSVE(
...
InstructionCost
PS2VPUTTIImpl::getArithmeticReductionCost(unsigned Opcode, VectorType *ValTy,
  ...
  // OR: llvm/test/CodeGen/PS2VPU/reduce-or.ll
  // XOR: llvm/test/CodeGen/PS2VPU/reduce-xor.ll
  // AND: llvm/test/CodeGen/PS2VPU/reduce-and.ll
...
InstructionCost PS2VPUTTIImpl::getSpliceCost(VectorType *Tp, int Index) {
  ...
  // Predicated splice are promoted when lowering. See PS2VPUISelLowering.cpp
...
InstructionCost PS2VPUTTIImpl::getShuffleCost(TTI::ShuffleKind Kind,
                                               VectorType *Tp,
                                               ArrayRef<int> Mask,
                                               TTI::TargetCostKind CostKind,
...
bool PS2VPUTTIImpl::preferPredicateOverEpilogue(TailFoldingInfo *TFI) {
  /*if (!ST->hasSVE())*/
    return false;
...
InstructionCost
PS2VPUTTIImpl::getScalingFactorCost(Type *Ty, GlobalValue *BaseGV,
