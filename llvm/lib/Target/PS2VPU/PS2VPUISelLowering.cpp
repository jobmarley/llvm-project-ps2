...
////===----------------------------------------------------------------------===//
//
//static bool CC_PS2VPU_Assign_SRet(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
...
//}
//
//static bool CC_PS2VPU_Assign_Split_64(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
//                                     CCValAssign::LocInfo &LocInfo,
...
//}
//
//static bool CC_PS2VPU_Assign_Ret_Split_64(unsigned &ValNo, MVT &ValVT,
//                                         MVT &LocVT,
//                                         CCValAssign::LocInfo &LocInfo,
//                                         ISD::ArgFlagsTy &ArgFlags,
//                                         CCState &State) {
...
//static bool CC_PS2VPU64_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
//                            CCValAssign::LocInfo &LocInfo,
//                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
//static bool CC_PS2VPU64_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
//                            CCValAssign::LocInfo &LocInfo,
//                            ISD::ArgFlagsTy &ArgFlags, CCState &State) {
...
//SDValue
//PS2VPUTargetLowering::LowerReturn_64(SDValue Chain, CallingConv::ID CallConv,
//                                    bool IsVarArg,
//                                    const SmallVectorImpl<ISD::OutputArg> &Outs,
//                                    const SmallVectorImpl<SDValue> &OutVals,
//                                    const SDLoc &DL, SelectionDAG &DAG) const {
...
//
//  // Analyze return values.
//  CCInfo.AnalyzeReturn(Outs, RetCC_PS2VPU64);
//
//  SDValue Flag;
//  SmallVector<SDValue, 4> RetOps(1, Chain);
//
...
//SDValue PS2VPUTargetLowering::LowerFormalArguments_64(
//    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
//    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
//    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
//  MachineFunction &MF = DAG.getMachineFunction();
//
//  // Analyze arguments according to CC_PS2VPU64.
//  SmallVector<CCValAssign, 16> ArgLocs;
//  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), ArgLocs,
//                 *DAG.getContext());
//  CCInfo.AnalyzeFormalArguments(Ins, CC_PS2VPU64);
//
...
//    assert(VA.isMemLoc());
//    // The CC_PS2VPU64_Full/Half functions compute stack offsets relative to the
//    // beginning of the arguments area at %fp+BIAS+128.
//    unsigned Offset = VA.getLocMemOffset() + ArgArea;
//    unsigned ValSize = VA.getValVT().getSizeInBits() / 8;
//    // Adjust offset for extended arguments, PS2VPU is big-endian.
...
PS2VPUTargetLowering::PS2VPUTargetLowering(const TargetMachine &TM,
  ...
  //// ATOMICs.
  //// Atomics are supported on PS2VPUV9. 32-bit atomics are also
  //// supported by some Leon PS2VPUV8 variants. Otherwise, atomics
  //// are unsupported.
  //if (Subtarget->isV9())
  ...
  //if (!Subtarget->isV9()) {
  //  // PS2VPUV8 does not have FNEGD and FABSD.
...
//SDValue PS2VPUTargetLowering::makeHiLoPair(SDValue Op, unsigned HiTF,
//                                          unsigned LoTF,
//                                          SelectionDAG &DAG) const {
//  SDLoc DL(Op);
...
//SDValue PS2VPUTargetLowering::makeAddress(SDValue Op, SelectionDAG &DAG) const {
//  SDLoc DL(Op);
//  EVT VT = getPointerTy(DAG.getDataLayout());
//
//  // Handle PIC mode first. PS2VPU needs a got load for every variable!
...
//                        withTargetFlags(Op, PS2VPUMCExpr::VK_PS2VPU_GOT13, DAG));
//    } else {
//      // This is the pic32 code model, the GOT is known to be smaller than 4GB.
//      Idx = makeHiLoPair(Op, PS2VPUMCExpr::VK_PS2VPU_GOT22,
//                         PS2VPUMCExpr::VK_PS2VPU_GOT10, DAG);
//    }
//
...
//    // abs32.
//    return makeHiLoPair(Op, PS2VPUMCExpr::VK_PS2VPU_HI, PS2VPUMCExpr::VK_PS2VPU_LO,
//                        DAG);
//  case CodeModel::Medium: {
//    // abs44.
//    SDValue H44 = makeHiLoPair(Op, PS2VPUMCExpr::VK_PS2VPU_H44,
//                               PS2VPUMCExpr::VK_PS2VPU_M44, DAG);
//    H44 = DAG.getNode(ISD::SHL, DL, VT, H44, DAG.getConstant(12, DL, MVT::i32));
//    SDValue L44 = withTargetFlags(Op, PS2VPUMCExpr::VK_PS2VPU_L44, DAG);
//    L44 = DAG.getNode(SPISD::Lo, DL, VT, L44);
//    return DAG.getNode(ISD::ADD, DL, VT, H44, L44);
//  }
//  case CodeModel::Large: {
//    // abs64.
//    SDValue Hi = makeHiLoPair(Op, PS2VPUMCExpr::VK_PS2VPU_HH,
//                              PS2VPUMCExpr::VK_PS2VPU_HM, DAG);
//    Hi = DAG.getNode(ISD::SHL, DL, VT, Hi, DAG.getConstant(32, DL, MVT::i32));
//    SDValue Lo = makeHiLoPair(Op, PS2VPUMCExpr::VK_PS2VPU_HI,
//                              PS2VPUMCExpr::VK_PS2VPU_LO, DAG);
//    return DAG.getNode(ISD::ADD, DL, VT, Hi, Lo);
//  }
//  }
//}
//
//SDValue PS2VPUTargetLowering::LowerGlobalAddress(SDValue Op,
//                                                SelectionDAG &DAG) const {
//  return makeAddress(Op, DAG);
//}
//
//SDValue PS2VPUTargetLowering::LowerConstantPool(SDValue Op,
//                                               SelectionDAG &DAG) const {
//  return makeAddress(Op, DAG);
//}
//
//SDValue PS2VPUTargetLowering::LowerBlockAddress(SDValue Op,
//                                               SelectionDAG &DAG) const {
//  return makeAddress(Op, DAG);
//}
//
//SDValue PS2VPUTargetLowering::LowerGlobalTLSAddress(SDValue Op,
//                                                   SelectionDAG &DAG) const {
//
...
//                         ? PS2VPUMCExpr::VK_PS2VPU_TLS_GD_HI22
//                         : PS2VPUMCExpr::VK_PS2VPU_TLS_LDM_HI22);
//    unsigned LoTF = ((model == TLSModel::GeneralDynamic)
//                         ? PS2VPUMCExpr::VK_PS2VPU_TLS_GD_LO10
//                         : PS2VPUMCExpr::VK_PS2VPU_TLS_LDM_LO10);
//    unsigned addTF = ((model == TLSModel::GeneralDynamic)
//                          ? PS2VPUMCExpr::VK_PS2VPU_TLS_GD_ADD
//                          : PS2VPUMCExpr::VK_PS2VPU_TLS_LDM_ADD);
//    unsigned callTF = ((model == TLSModel::GeneralDynamic)
//                           ? PS2VPUMCExpr::VK_PS2VPU_TLS_GD_CALL
//                           : PS2VPUMCExpr::VK_PS2VPU_TLS_LDM_CALL);
//
...
//        SPISD::Hi, DL, PtrVT,
//        withTargetFlags(Op, PS2VPUMCExpr::VK_PS2VPU_TLS_LDO_HIX22, DAG));
//    SDValue Lo = DAG.getNode(
//        SPISD::Lo, DL, PtrVT,
//        withTargetFlags(Op, PS2VPUMCExpr::VK_PS2VPU_TLS_LDO_LOX10, DAG));
//    HiLo = DAG.getNode(ISD::XOR, DL, PtrVT, Hi, Lo);
//    return DAG.getNode(
//        SPISD::TLS_ADD, DL, PtrVT, Ret, HiLo,
//        withTargetFlags(Op, PS2VPUMCExpr::VK_PS2VPU_TLS_LDO_ADD, DAG));
//  }
//
//  if (model == TLSModel::InitialExec) {
//    unsigned ldTF = ((PtrVT == MVT::i64) ? PS2VPUMCExpr::VK_PS2VPU_TLS_IE_LDX
//                                         : PS2VPUMCExpr::VK_PS2VPU_TLS_IE_LD);
//
//    SDValue Base = DAG.getNode(SPISD::GLOBAL_BASE_REG, DL, PtrVT);
//
//    // GLOBAL_BASE_REG codegen'ed with call. Inform MFI that this
...
//    MFI.setHasCalls(true);
//
//    SDValue TGA = makeHiLoPair(Op, PS2VPUMCExpr::VK_PS2VPU_TLS_IE_HI22,
//                               PS2VPUMCExpr::VK_PS2VPU_TLS_IE_LO10, DAG);
...
//        withTargetFlags(Op, PS2VPUMCExpr::VK_PS2VPU_TLS_IE_ADD, DAG));
//  }
//
//  assert(model == TLSModel::LocalExec);
//  SDValue Hi =
//      DAG.getNode(SPISD::Hi, DL, PtrVT,
//                  withTargetFlags(Op, PS2VPUMCExpr::VK_PS2VPU_TLS_LE_HIX22, DAG));
//  SDValue Lo =
//      DAG.getNode(SPISD::Lo, DL, PtrVT,
//                  withTargetFlags(Op, PS2VPUMCExpr::VK_PS2VPU_TLS_LE_LOX10, DAG));
...
//}
//
//SDValue PS2VPUTargetLowering::LowerF128_LibCallArg(SDValue Chain,
...
//}
//
//SDValue PS2VPUTargetLowering::LowerF128Op(SDValue Op, SelectionDAG &DAG,
...
//}
//
//SDValue PS2VPUTargetLowering::LowerF128Compare(SDValue LHS, SDValue RHS,
...
//    regSpillArea = 128;
//  } else {
//    // On PS2VPU32, the size of the spill area is 92. Unfortunately,
...
//}
//
//// Custom lower UMULO/SMULO for PS2VPU. This code is similar to ExpandNode()
...
//}
//
//SDValue PS2VPUTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
//                                                     SelectionDAG &DAG) const {
...
//
////===----------------------------------------------------------------------===//
////                         PS2VPU Inline Assembly Support
////===----------------------------------------------------------------------===//
//
///// getConstraintType - Given a constraint letter, return the type of
///// constraint it is for this target.
//PS2VPUTargetLowering::ConstraintType
//PS2VPUTargetLowering::getConstraintType(StringRef Constraint) const {
...
//PS2VPUTargetLowering::getSingleConstraintMatchWeight(
...
//void PS2VPUTargetLowering::LowerAsmOperandForConstraint(
...
//PS2VPUTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
