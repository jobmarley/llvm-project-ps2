//===- PS2VPUTargetTransformInfo.h - PS2VPU specific TTI ------*- C++ -*-===//
//
...
/// PS2VPU target machine. It uses the target's detailed information to
/// provide more precise answers to certain TTI queries, while letting the
/// target independent and default TTI implementations handle the rest.
///
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_PS2VPU_PS2VPUTARGETTRANSFORMINFO_H
#define LLVM_LIB_TARGET_PS2VPU_PS2VPUTARGETTRANSFORMINFO_H
...
class PS2VPUTTIImpl : public BasicTTIImplBase<PS2VPUTTIImpl> {
  using BaseT = BasicTTIImplBase<PS2VPUTTIImpl>;
  using TTI = TargetTransformInfo;

  friend BaseT;

  const PS2VPUSubtarget *ST;
  const PS2VPUTargetLowering *TLI;

  const PS2VPUSubtarget *getST() const { return ST; }
  const PS2VPUTargetLowering *getTLI() const { return TLI; }
...
public:
  explicit PS2VPUTTIImpl(const PS2VPUTargetMachine *TM, const Function &F)
  ...
  unsigned getMaxNumElements(ElementCount VF) const {
    ...
    return VF.getKnownMinValue() * 2; // ST->getVScaleForTuning();
  }
...
#endif // LLVM_LIB_TARGET_PS2VPU_PS2VPUTARGETTRANSFORMINFO_H
